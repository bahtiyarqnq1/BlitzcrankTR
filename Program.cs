    ...
    public static class Program
    {
        public static void Main(string[] args)
        {
            Loading.OnLoadingComplete += delegate
            {
                ...
                menu.AddGroupLabel("Hitchance(Q tuturma yuzdesı)");
                var hitchances = new List<HitChance>();
                for (var i = (int)HitChance.Dashing; i <= (int)HitChance.Immobile; i++)
                {
                ...
                menu.Add("hitchance(Q tuturma yuzdesı)", slider);
                ...
                var predictions = menu.Add("predictions", new CheckBox("Rakibın altında cekme yuzdesını goster"));

                #endregion

                var Q = new Spell.Skillshot(SpellSlot.Q, 810, SkillShotType.Linear, 250, 1800, 70);
                ...
                Drawing.OnDraw += delegate
                {
                    ...
                    foreach (var prediction in predictedPositions.ToArray())
                    {
                        ...
                        Drawing.DrawText(prediction.Value.Item2.CastPosition.WorldToScreen() + new Vector2(0, -20), System.Drawing.Color.LimeGreen,
                            string.Format("Hitchance: {90}%", Math.Ceiling(prediction.Value.Item2.HitChancePercent)),90);
                    }
                };
            };
        }
